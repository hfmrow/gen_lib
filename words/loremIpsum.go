...
func LoremGenerateTextFile(filename string, maxWords, maxChars int) error {
	return ioutil.WriteFile(filename, []byte(gst.FormatText(Lorem(maxWords), maxChars, false)), os.ModePerm)
...
func Lorem(wordCount ...int) string {
	...
	tmpSentence[len(tmpSentence)-1] = gst.RemoveNonAlNum(tmpSentence[len(tmpSentence)-1]) + "."
