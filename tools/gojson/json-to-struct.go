				...
				if sub != "" {
					...
					if subStructMap != nil {
						if val, ok := subStructMap[sub]; ok {
							...
							// H.F.M: Sub structures get parent name
							// inspired by; #72 opened on 8 Dec 2018 by lobatt
...
// H.F.M added on 2019-09-29 to meet my need,
// Setting package name as "" to get only structure
// Input as []byte rather than an io.Reader.
// A workaround to solve issue when struct start
// as a slice and sub-struct is set at true.
func JsonToStruct(data []byte, structName, pkgName string, subStruct, convertFloat bool) (out []byte, err error) {
	var changed, noPkgName bool
	// Remove [] when they enclose the structure
	if string(data[0]) == "[" && string(data[len(data)-1]) == "]" && subStruct {
	...
	if out, err = Generate(r, ParseJson, structName, pkgName, []string{"json"}, subStruct, convertFloat); err == nil {
		if noPkgName {
			// Delete "package name" if it is not specified.
			out = bytes.Replace(out, []byte("package "+pkgName+"\n\n"), []byte(""), 1)
		}
		if changed {
			// Restore [] after conversion
			out = bytes.Replace(out, []byte("struct"), []byte("[]struct"), 1)
		}
	}
	return
}
